...
function initial() 
{
    ...
    $sql="SELECT tblUsuario.id_usuario, tblUsuario.nombre, tblUsuario.password, tblUsuario.usuario, tblUsuario.id_tipo_usuario_USU, tblEmpleado.nombre as 'nombreEM', tblEmpleado.apellido as 'apellidoEM' FROM tblUsuario INNER JOIN tblEmpleado on tblUsuario.id_empleado_usuario = tblEmpleado.id_empleado WHERE id_usuario ='$id_usuario'";
    $result=_query($sql);
                      ...
                      <?php
				      	if ($count>0)
				      	{
					        for($i=0;$i<$count;$i++)
					        {
					            ...
					            $tipo_usuario=$row['id_tipo_usuario_USU'];
								$administrador=$row['id_tipo_usuario_USU'];
					        } 
				        }          
				       ?>
				    <div class="row">                                           
