		 			...
		 			if($("#forma").val() == "constancia")
		 			{
		 				if($("#id_paciente").val() != "")
	 					{
			 				if($("#padecimiento").val() != "")
			 				{
				 				...
				 				{
				 					display_notify("Error", "Por favor ingrese el tratamiento");
				 				}
			 				}
			 				else
			 				{
...
function senddata()
{
	...
	var process=$('#process').val();
		var dataString ='process='+process+'&id_paciente='+id_paciente+'&fecha='+fecha+'&padecimiento='+padecimiento+"&tratamiento="+tratamiento;
			dataString += "&reposo="+reposo+"&fecha_d="+fecha_d+"&hora="+hora+"&lugar="+lugar+"&causa="+causa+"&tipo="+forma+"&texto="+texto;
	$.ajax({
        type: "POST",
        url: "constancias.php",
        data: dataString,
...
function imprimir_constancia(id)
{
	...
	url=encabezado+'?id_constancia='+id;
	window.open(url, '', '');
}
...
function generar2(){
	...
	dataTable = $('#editable2').DataTable( {
			...
			"serverSide": true,
			"ajax":{
					url :"admin_constancia_dt.php?fechai="+fechai+"&fechaf="+fechaf, // json datasource
