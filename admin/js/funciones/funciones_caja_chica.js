$(document).ready(function() {
	...
	$('#valor').numeric({
			  ...
			  if(concepto!=""){
				  if(monto!=""){
					  if(recibe!=""){
						  ...
						  display_notify('Warning', 'Recibe  es requerido!');
					  }
				  }else{
					  display_notify('Warning', 'El monto es requerido!');
				  }
			  }else{
				  display_notify('Warning', 'El concepto es requerido!');
			  }
		  }else{
			  display_notify('Warning', 'El número de documento es requerido!');
		  }
	  ...
	  if ($("#presentacion_table tr").length > 0) {
		  ...
		  dataString += '&turno=' + turno + '&cuantos='+ cuantos+'&id_movimiento='+id_movimiento+"&monto_anti="+monto_anti+"&n_doc="+n_doc;
		if(concepto){
		if(monto){
		if(monto){
		$.ajax({
	...
	function imprimir_viatico(id_movimiento){
	  ...
	  $("#search").click(function()
	  {
		  ...
		  dataTable = $('#editable_caja_c').DataTable().destroy()
		  dataTable = $('#editable_caja_c').DataTable( {
			  "pageLength": 50,
			  "responsive": true,
			  "autoWidth": false,
			  "order":[ 0, 'asc' ],
			  ...
			  "serverSide": true,
			  "ajax":{
					  url :"admin_movimiento_caja_dt.php?fecha1="+fecha1+"&fecha2="+fecha2, // json datasource
					  //url :"admin_factura_rangos_dt.php", // json datasource
					  //type: "post",  // method  , by default get
					  error: function(){  // error handling
						  $(".editable_caja_c-error").html("");
						  $("#editable_caja_c").append('<tbody class="editable_grid-error"><tr><th colspan="3">No se encontró información segun busqueda </th></tr></tbody>');
						  $("#editable_caja_c_processing").css("display","none");
						  $( ".editable_caja_c-error" ).remove();
						  }
					  }
				  } );
